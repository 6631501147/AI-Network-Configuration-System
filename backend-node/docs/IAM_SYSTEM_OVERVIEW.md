# IAM System Overview — AIBasedAutomaticNetworkConfigurationSystem Backend Node.js

> เอกสารนี้สรุปสถาปัตยกรรม วิธีการทำงาน รูปแบบการเขียนโค้ด และข้อกำหนดของระบบ IAM ใน AIBasedAutomaticNetworkConfigurationSystem Backend
...
AIBasedAutomaticNetworkConfigurationSystem Backend ใช้ระบบ IAM (Identity and Access Management) แบบ **Dual Permission Model** ซึ่งสามารถทำงานได้ 2 โหมด:
...
| **Local Fallback** | `PROJECT_PERMISSION_SOURCE=local` | ใช้ฐานข้อมูล MongoDB ของ AIBasedAutomaticNetworkConfigurationSystem เอง |
...
- รองรับ B2B Token สำหรับ AIBasedAutomaticNetworkConfigurationSystem API
...
```
┌─────────────────────────────────────────────────────────┐
│                   AIBasedAutomaticNetworkConfigurationSystem Backend                        │
│                                                          │
│  ┌──────────┐    ┌──────────────┐    ┌───────────────┐  │
...
|-----------|------|---------|
| IAM SDK | `server/integrations/iam/sdk.js` | Factory สร้าง SDK instance สำหรับ AIBasedAutomaticNetworkConfigurationSystem |
...
| B2B Auth | `aibasedautomaticnetworkconfigurationsystem/service/aibasedautomaticnetworkconfigurationsystem_document.js` | ตรวจสอบ B2B Token สำหรับ AIBasedAutomaticNetworkConfigurationSystem API |
...
- สร้าง AIBasedAutomaticNetworkConfigurationSystem-configured IAM SDK instance ด้วย `createAIBasedAutomaticNetworkConfigurationSystemSdk()`
...
| `forwardAccountsList()` | List accounts พร้อม AIBasedAutomaticNetworkConfigurationSystem scope filter |
| `resolveScopedSecurityMetadata()` | กรอง Groups/Menus ตาม AIBasedAutomaticNetworkConfigurationSystem scope |
| `inviteAccountToScope()` | เชิญ user ใหม่เข้า AIBasedAutomaticNetworkConfigurationSystem scope |
| `syncAccountAssignments()` | Sync group memberships |
| `removeAccountFromScope()` | ปิดการเข้าถึง AIBasedAutomaticNetworkConfigurationSystem ทั้งหมดของ account |
...
```
Client → GET /api/v1/aibasedautomaticnetworkconfigurationsystem/documents/...
  → onCheckAuthorization() → ดึง account จาก x-access-token
  → requirePermission('/ai-based-automatic-network-configuration-system/registry', 'view')
    → normalizePath('/ai-based-automatic-network-configuration-system/registry')
    ...
    → ตรวจสอบ matrix['/ai-based-automatic-network-configuration-system/registry']['view'] === true
  → ถ้าผ่าน: next()
...
### B2B Token Flow
```
External System → POST /api/v1/aibasedautomaticnetworkconfigurationsystem/documents/...
  + Authorization: Bearer <token>
  → onCheckB2BAuthorization()
    → introspectToken() → IAM Service
    → ตรวจสอบ active, audience, scope
  → requireScope('ai.based.automatic.network.configuration.system.registry.write')
    → ตรวจ token.scopes ประกอบด้วย 'ai.based.automatic.network.configuration.system.registry.write'
  → ถ้าผ่าน: next()
```
...
| `IAM_SDK_CLIENT_ID` | `aibasedautomaticnetworkconfigurationsystem-api` | Client ID สำหรับ IAM |
...
| `PROJECT_PERMISSION_TYPE_TITLE` | `AI Based Automatic Network Configuration System Administration` | ชื่อ type ใน IAM |
...
```
ai.based.automatic.network.configuration.system.registry.read ai.based.automatic.network.configuration.system.registry.write ai.based.automatic.network.configuration.system.report.read
