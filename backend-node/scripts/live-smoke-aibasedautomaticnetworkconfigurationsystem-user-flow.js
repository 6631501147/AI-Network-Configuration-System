...
function createHttpClient(baseUrl, token) {
  return axios.create({
    ...
    headers: {
      ...
      'user-agent': 'aibasedautomaticnetworkconfigurationsystem-live-smoke/1.0',
      lang: 'en'
    },
...
async function createTemporaryIamSession() {
  ...
  const token = 'aibasedautomaticnetworkconfigurationsystem-live-smoke-' + crypto.randomBytes(16).toString('hex');
  const now = new Date();
  const expiresAtUnix = Math.floor(Date.now() / 1000) + 1800;
  const sessionPayload = {
    version: '1',
    ip: '127.0.0.1',
    device: 'aibasedautomaticnetworkconfigurationsystem-live-smoke-script',
    dateTime: now,
    xAccessToken: token,
    expired_key: expiresAtUnix,
    deviceId: 'aibasedautomaticnetworkconfigurationsystem-live-smoke-device',
    fingerprint: crypto.createHash('sha256').update('aibasedautomaticnetworkconfigurationsystem-live-smoke-device|aibasedautomaticnetworkconfigurationsystem-live-smoke-script').digest('hex'),
    networkKey: '127.0.0',
