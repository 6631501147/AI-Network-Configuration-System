...
const AIBasedAutomaticNetworkConfigurationSystemDocument = require('../models/aibasedautomaticnetworkconfigurationsystem_document.model');
...
function payloadFromBody(body) {
  const payload = {
    aibasedautomaticnetworkconfigurationsystemNo: cleanText(body.aibasedautomaticnetworkconfigurationsystemNo),
...
function buildListQuery(query) {
  ...
  if (q) {
    filter.$or = [
      { aibasedautomaticnetworkconfigurationsystemNo: new RegExp(q, 'i') },
...
exports.list = async function list(query) {
  ...
  const [rows, total] = await Promise.all([
    AIBasedAutomaticNetworkConfigurationSystemDocument.find(filter).sort({ expiryDate: 1, updatedAt: -1 }).skip(skip).limit(limit).lean(),
    AIBasedAutomaticNetworkConfigurationSystemDocument.countDocuments(filter)
  ]);
...
exports.stats = async function stats() {
  ...
  const [total, active, review, expiring, expired] = await Promise.all([
    AIBasedAutomaticNetworkConfigurationSystemDocument.countDocuments({}),
    AIBasedAutomaticNetworkConfigurationSystemDocument.countDocuments({ status: 'active' }),
    AIBasedAutomaticNetworkConfigurationSystemDocument.countDocuments({ status: 'review' }),
    AIBasedAutomaticNetworkConfigurationSystemDocument.countDocuments({
      $or: [
        { status: 'expiring' },
...
exports.create = async function create(body, request) {
  ...
  const missing = ['aibasedautomaticnetworkconfigurationsystemNo', 'title', 'partnerName'].filter(function (field) {
  ...
  const created = await AIBasedAutomaticNetworkConfigurationSystemDocument.create(updateLifecycleStatus(payload));
  return created.toObject();
};

exports.update = async function update(id, body, request) {
  if (!mongoose.Types.ObjectId.isValid(id)) {
    const error = new Error('Invalid AIBasedAutomaticNetworkConfigurationSystem document id');
    error.status = 400;
    throw error;
  }
  ...
  const updated = await AIBasedAutomaticNetworkConfigurationSystemDocument.findOneAndUpdate(
  ...
  if (!updated) {
    const error = new Error('AIBasedAutomaticNetworkConfigurationSystem document not found');
    error.status = 404;
    throw error;
  }
  return updated;
...
exports.remove = async function remove(id) {
  ...
  const result = await AIBasedAutomaticNetworkConfigurationSystemDocument.deleteOne({ _id: new mongoose.Types.ObjectId(id) });
  return { deleted: result.deletedCount || 0 };
};

exports.seedDemo = async function seedDemo(request) {
  const existing = await AIBasedAutomaticNetworkConfigurationSystemDocument.countDocuments({});
  if (existing > 0) {
  ...
  await AIBasedAutomaticNetworkConfigurationSystemDocument.insertMany([
    {
      aibasedautomaticnetworkconfigurationsystemNo: 'AIBasedAutomaticNetworkConfigurationSystem-INT-2026-001',
      ...
      coordinatorName: 'AIBasedAutomaticNetworkConfigurationSystem Coordinator',
      coordinatorEmail: 'aibasedautomaticnetworkconfigurationsystem@mfu.ac.th',
      status: 'active',
      effectiveDate: today,
      expiryDate: monthsFromNow(30),
      renewalNoticeDate: monthsFromNow(27),
      tags: ['academic', 'exchange'],
      notes: 'Initial demo record for validating AIBasedAutomaticNetworkConfigurationSystem workflows.',
      create: actor
    },
    {
      aibasedautomaticnetworkconfigurationsystemNo: 'AIBasedAutomaticNetworkConfigurationSystem-RES-2026-002',
    ...
    {
      aibasedautomaticnetworkconfigurationsystemNo: 'AIBasedAutomaticNetworkConfigurationSystem-EXP-2026-003',
