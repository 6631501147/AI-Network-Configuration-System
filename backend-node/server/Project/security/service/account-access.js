...
async function findAIBasedAutomaticNetworkConfigurationSystemAccountById(accountId) {
...
async function resolveIamAccountIdByAIBasedAutomaticNetworkConfigurationSystemAccountId(accountId) {
  const aibasedautomaticnetworkconfigurationsystemAccount = await findAIBasedAutomaticNetworkConfigurationSystemAccountById(accountId);
  if (!aibasedautomaticnetworkconfigurationsystemAccount) return String(accountId || '');
  const email = aibasedautomaticnetworkconfigurationsystemAccount && aibasedautomaticnetworkconfigurationsystemAccount.email ? String(aibasedautomaticnetworkconfigurationsystemAccount.email).trim().toLowerCase() : '';
  if (!email) return String(accountId || '');
  const sdk = createAIBasedAutomaticNetworkConfigurationSystemSdk();
...
async function getIamPermissionData(accountId, options) {
  ...
  if (options && options.targetAccountId) {
    targetIamAccountId = await resolveIamAccountIdByAIBasedAutomaticNetworkConfigurationSystemAccountId(options.targetAccountId);
...
async function loadAssignmentsByAccountIds(accountIds) {
  ...
  if (useIamPermissionSource()) {
    const sdk = createAIBasedAutomaticNetworkConfigurationSystemSdk();
    const rows = await Promise.all(ids.map(async function (aibasedautomaticnetworkconfigurationsystemAccountId) {
      try {
        const iamAccountId = await resolveIamAccountIdByAIBasedAutomaticNetworkConfigurationSystemAccountId(aibasedautomaticnetworkconfigurationsystemAccountId);
        if (!iamAccountId) {
          return [String(aibasedautomaticnetworkconfigurationsystemAccountId), []];
        }
        ...
        return [String(aibasedautomaticnetworkconfigurationsystemAccountId), assignments];
      } catch (error) {
        return [String(aibasedautomaticnetworkconfigurationsystemAccountId), null];
      }
    }));
