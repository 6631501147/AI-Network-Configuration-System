...
const { createAIBasedAutomaticNetworkConfigurationSystemSdk } = require('../../../integrations/iam/sdk');
...
function createUserRequestOptions(request, options) {
  ...
  return {
    ...
    headers: {
      ...
      'x-system': config.project && config.project.code ? String(config.project.code) : 'aibasedautomaticnetworkconfigurationsystem'
    }
  };
}
...
async function loadCurrentAIBasedAutomaticNetworkConfigurationSystemAccount(request) {
...
async function loadAIBasedAutomaticNetworkConfigurationSystemAccountById(accountId) {
  if (!accountId) return null;
  try {
...
async function resolveIamAccountIdByEmail(email) {
  if (!email) return '';
  ensureAdminConfig();
  const iamSdk = createAIBasedAutomaticNetworkConfigurationSystemSdk();
...
async function resolveIamAccountIdByAIBasedAutomaticNetworkConfigurationSystemAccountId(accountId) {
  const aibasedautomaticnetworkconfigurationsystemAccount = await loadAIBasedAutomaticNetworkConfigurationSystemAccountById(accountId);
  if (!aibasedautomaticnetworkconfigurationsystemAccount) return String(accountId || '');
  const email = aibasedautomaticnetworkconfigurationsystemAccount && aibasedautomaticnetworkconfigurationsystemAccount.email ? aibasedautomaticnetworkconfigurationsystemAccount.email : '';
  ...
  try {
    ...
    if (!allowed) {
      ...
      return response.status(403).json({
        status: false,
        error: 'account_not_in_aibasedautomaticnetworkconfigurationsystem_scope'
      });
    }
...
async function forwardMyPermissions(request, response) {
  try {
    const aibasedautomaticnetworkconfigurationsystemAccount = await loadCurrentAIBasedAutomaticNetworkConfigurationSystemAccount(request);
    const email = aibasedautomaticnetworkconfigurationsystemAccount && aibasedautomaticnetworkconfigurationsystemAccount.email ? aibasedautomaticnetworkconfigurationsystemAccount.email : '';
    if (!email) {
...
async function inviteAccountToScope(payload, headers) {
  ...
  const groupIds = Array.isArray(body.groupIds)
    ...
    : [];
  if (!groupIds.length) {
    const error = new Error('aibasedautomaticnetworkconfigurationsystem_group_required');
    error.statusCode = 400;
    error.payload = { status: false, error: 'aibasedautomaticnetworkconfigurationsystem_group_required' };
    throw error;
  }
...
async function syncAccountAssignments(accountId, groupIds) {
  const iamAccountId = await resolveIamAccountIdByAIBasedAutomaticNetworkConfigurationSystemAccountId(accountId);
  if (!iamAccountId) {
    return {
      accountId: null,
      assignments: [],
...
module.exports = {
  ...
  resolveIamAccountIdByAIBasedAutomaticNetworkConfigurationSystemAccountId,
  requestAdmin,
  requestUser,
  forwardAdminRequest,
