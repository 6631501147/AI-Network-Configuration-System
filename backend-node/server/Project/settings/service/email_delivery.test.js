...
test('pay hub email delivery service scopes get requests to aibasedautomaticnetworkconfigurationsystem', async function () {
    let requestOptions = null;
    iamAdminClient.requestAdmin = async function (options) {
        requestOptions = options;
        return { number: 0, code: 20000, data: { appId: 'aibasedautomaticnetworkconfigurationsystem' } };
    };
...
test('pay hub email delivery service scopes update payloads to aibasedautomaticnetworkconfigurationsystem', async function () {
    let requestOptions = null;
    iamAdminClient.requestAdmin = async function (options) {
        requestOptions = options;
    ...
    await emailDeliveryService.onUpdate({
        body: {
            ...
            smtp: {
                host: 'smtp.aibasedautomaticnetworkconfigurationsystem.example.test'
            }
        },
        headers: {
            lang: 'th'
    ...
    assert.equal(requestOptions.data.appId, 'aibasedautomaticnetworkconfigurationsystem');
    assert.equal(requestOptions.data.appName, 'AIBasedAutomaticNetworkConfigurationSystem');
});
