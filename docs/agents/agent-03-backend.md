...
| response | legacy modules often use settings message response service; `aibasedautomaticnetworkconfigurationsystem` routes use direct `{ code, message, data }` JSON |
...
| Mount | Module |
|---|---|
| `/api/v1/aibasedautomaticnetworkconfigurationsystem` | `Project/aibasedautomaticnetworkconfigurationsystem/aibasedautomaticnetworkconfigurationsystem.routes.js` |
...
Note: `frontend-vue/src/service/api.js` currently calls the document API with lowercase `/api/v1/aibasedautomaticnetworkconfigurationsystem`. Verify Express case-sensitivity and preserve compatibility before changing route casing.
...
- document registry mutation: `edit/delete` on `/ai-based-automatic-network-configuration-system/registry`
...
```txt
ทำหน้าที่ Backend Agent สำหรับ AIBasedAutomaticNetworkConfigurationSystem
