# T1-T20 Change Document: AIBasedAutomaticNetworkConfigurationSystem Auth Bootstrap And Permission Latency

## T1 Change Title

| Field | Value |
|---|---|
| Change ID | AIBasedAutomaticNetworkConfigurationSystem-AUTH-PERF-20260605 |
| Module | AIBasedAutomaticNetworkConfigurationSystem frontend auth/session/security |
| Date | 2026-06-05 |
...
- Server deploy completed on `192.168.17.25`; `aibasedautomaticnetworkconfigurationsystem-backend-1` reported healthy and `aibasedautomaticnetworkconfigurationsystem-frontend-1` was recreated after syncing `frontend-vue/src/projects/utils/permission-landing.js`.
- Server smoke passed: `http://127.0.0.1:8086/` returned 200 in about 0.002s; `http://127.0.0.1:8215/api/v1/auth/me` returned unauthenticated 401 in about 0.015s.
- Public smoke blocked: `https://ai-based-automatic-network-configuration-system.mfu.ac.th/` and `https://ai-based-automatic-network-configuration-system.mfu.ac.th/api/v1/auth/me` did not resolve from this machine.
- Watchman recrawl warning appeared during tests; it did not fail tests.

## T17 PRD / Docs Updated

- `docs/prd/PRD-AIBasedAutomaticNetworkConfigurationSystem.md` updated.
...
| B-001 | Blocker | `ai-based-automatic-network-configuration-system.mfu.ac.th` DNS did not resolve from this machine during public smoke. | open |
| D-001 | Decision | Use session hint as the signal for cookie-based IAM restore. | closed |

## T19 Release / Rollback

- Release: rebuilt and deployed AIBasedAutomaticNetworkConfigurationSystem frontend on `192.168.17.25`; public DNS still needs Ops confirmation.
...
Status: tested and deployed on the server. Public DNS for `ai-based-automatic-network-configuration-system.mfu.ac.th` remains blocked from this machine. No backend or data migration required.
