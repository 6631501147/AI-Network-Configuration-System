...
export default {
  ...
  computed: {
    ...
    sessionRows () {
      return (Array.isArray(this.sessions) ? this.sessions : []).map(item => ({
        ...
        system: item && item.system ? String(item.system) : 'aibasedautomaticnetworkconfigurationsystem',
  ...
  methods: {
    getSystemLabel (system) {
      const normalized = system ? String(system).trim() : 'aibasedautomaticnetworkconfigurationsystem'
      return normalized ? normalized.toUpperCase() : 'AIBasedAutomaticNetworkConfigurationSystem'
    },
    grantLabels (item) {
      const labels = []
