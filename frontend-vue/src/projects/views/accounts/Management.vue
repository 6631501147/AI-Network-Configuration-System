<template>
  <div class="account-page">
    ...
    <AccountDirectoryTable
      ...
      @remove-aibasedautomaticnetworkconfigurationsystem-access="removeAIBasedAutomaticNetworkConfigurationSystemAccess"
...
export default {
  ...
  methods: {
    ...
    async removeAIBasedAutomaticNetworkConfigurationSystemAccess (item) {
      if (!(item && item._id)) return
      try {
        ...
        if (!confirmed) return
        await this.$store.dispatch('accounts/removeAIBasedAutomaticNetworkConfigurationSystemAccess', item)
