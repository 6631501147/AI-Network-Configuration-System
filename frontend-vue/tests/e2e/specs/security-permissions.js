const { bootstrapAuthenticatedRoute } = require('../support/aibasedautomaticnetworkconfigurationsystem-session')

module.exports = {
  'AIBasedAutomaticNetworkConfigurationSystem security pages load IAM-backed data and save permission matrix rows': function (browser) {
  ...
  'AIBasedAutomaticNetworkConfigurationSystem security menu page keeps newly-created scoped types and menus visible': function (browser) {
    const devServer = process.env.VUE_DEV_SERVER_URL
    ...
    browser
      ...
      .executeAsync(function (done) {
        ...
        app.$store.dispatch('security/menu/createType', {
          title: [{ key: 'en', value: 'Custom AIBasedAutomaticNetworkConfigurationSystem Tools' }],
          description: [{ key: 'en', value: 'Created from AIBasedAutomaticNetworkConfigurationSystem E2E' }],
          state: true
        }).then(function (type) {
          return app.$store.dispatch('security/menu/createMenu', {
            title: [{ key: 'en', value: 'Custom AIBasedAutomaticNetworkConfigurationSystem Tool' }],
            description: [{ key: 'en', value: 'Custom menu created from AIBasedAutomaticNetworkConfigurationSystem' }],
            path: '/custom/aibasedautomaticnetworkconfigurationsystem-tool',
        ...
        browser.assert.ok((result.value.types || []).some(function (name) { return name === 'Custom AIBasedAutomaticNetworkConfigurationSystem Tools' }), 'created type remains visible')
        browser.assert.ok((result.value.menus || []).some(function (path) { return path === '/custom/aibasedautomaticnetworkconfigurationsystem-tool' }), 'created menu remains visible')
      })
      .end()
  }
}
