function bootstrapAuthenticatedRoute(browser, options) {
  var settings = Object.assign({
    route: '/dashboard',
    token: 'aibasedautomaticnetworkconfigurationsystem-e2e-token',
    profile: {
      _id: 'acc-1',
  ...
  return browser.executeAsync(function (bootstrapOptions, done) {
    ...
    function installDispatchShim(app, shimState) {
      ...
      app.$store.__aibasedautomaticnetworkconfigurationsystemE2EState = Object.assign({}, shimState || {});

      if (app.$store.__aibasedautomaticnetworkconfigurationsystemOriginalDispatch) {
        return;
      }

      app.$store.__aibasedautomaticnetworkconfigurationsystemOriginalDispatch = app.$store.dispatch.bind(app.$store);
      app.$store.dispatch = function (type, payload) {
        var currentShimState = this.__aibasedautomaticnetworkconfigurationsystemE2EState || {};
        ...
        return this.__aibasedautomaticnetworkconfigurationsystemOriginalDispatch(type, payload);
      };
    }
